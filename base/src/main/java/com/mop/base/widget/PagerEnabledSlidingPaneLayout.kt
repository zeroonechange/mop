    ...
    override fun onInterceptTouchEvent(ev: MotionEvent): Boolean {
        when (MotionEventCompat.getActionMasked(ev)) {
            ...
            MotionEvent.ACTION_MOVE -> {
                val x = ev.x
                val y = ev.y // The user should always be able to "commonx_close" the pane, so we only check
                // for child scrollability if the pane is currently closed.
                if (mInitialMotionX > mEdgeSlop && !isOpen && canScroll(
                        this, false, (x - mInitialMotionX).roundToInt(), x.roundToInt(), y.roundToInt()
                    )
                ) {
