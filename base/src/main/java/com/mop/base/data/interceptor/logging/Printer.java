package com.mop.base.data.interceptor.logging;
...
class Printer {
    ...
    static void printFileRequest(LoggingInterceptor.Builder builder, Request request) {
        ...
        String url;
        try {
            url = URLDecoder.decode(request.url().toString(), "UTF_8");
        } catch (UnsupportedEncodingException e) {
            url = request.url().toString();
        }
        logLines(builder.getType(), tag, new String[]{URL_TAG + url}, builder.getLogger(), false);
        logLines(builder.getType(), tag, getRequest(request, builder.getLevel()), builder.getLogger(), true);
        if (request.body() instanceof FormBody) {
            StringBuilder formBody = new StringBuilder();
            FormBody body = (FormBody) request.body();
