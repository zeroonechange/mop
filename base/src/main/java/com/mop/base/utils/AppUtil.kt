...
object AppUtil {
            ...
            try {
                return packageManager.getApplicationLabel(
                    packageManager.getApplicationInfo(
                        BaseApp.getInstance().packageName, 0
                    )
                ).toString()
            } catch (e: PackageManager.NameNotFoundException) {
                e.printStackTrace()
    ...
    fun isAppExist(packageName: String): Boolean {
        val packageManager = BaseApp.getInstance().packageManager // 获取所有已安装程序的包信息
    ...
    fun startActivityForResult(
        context: Activity, cls: Class<out Activity>, requestCode: Int, name: String, value: String
    ) {
        val intent = Intent(context, cls)
        intent.putExtra(name, value)
        context.startActivityForResult(intent, requestCode)
    ...
    private val mMetaData by lazy {
        BaseApp.getInstance().packageManager.getApplicationInfo(
            BaseApp.getInstance().packageName, PackageManager.GET_META_DATA
        ).metaData
    }
    ...
    fun getAppNameAndVersion(context: Context): String {
        ...
        val appName = context.resources.getString(packageInfo!!.applicationInfo.labelRes)
