...
object FileUtil {
    ...
    val appDir: String
        get() = if (isSDCardMounted) sdCardBaseDir + "/" + AppUtil.defPackageInfo!!.packageName + "/"
        else BaseApp.getInstance().filesDir.absolutePath + "/"

    // 取得App的文件目录，/sdcard/包名/File，没有sd卡则是/data/data/包名/files/File
    val appFileDir: String
        get() {
            val s = "/File/"
            return if (isSDCardMounted) sdCardBaseDir + "/" + AppUtil.defPackageInfo!!.packageName + s
            else BaseApp.getInstance().filesDir.absolutePath + s
        }
    ...
    val appLogDir: String
        get() {
            val s = "/Log/"
            val path = if (isSDCardMounted) cacheDir.absolutePath + s
            else BaseApp.getInstance().filesDir.absolutePath + s
            val file = File(path)
            if (!file.exists()) {
                file.mkdirs()
    ...
    fun getDirLength(dir: File): Long {
        var length: Long = 0 // 删除文件夹中的所有文件包括子目录
    ...
    private fun saveStrToLocal(filePath: String, str: String, append: Boolean): Boolean {
        ...
        if (!file.parentFile.exists()) // 文件路径不存在则创建
        {
            if (!file.parentFile.mkdirs()) // 创建失败直接返回
            {
                return false
            }
        }
    ...
    fun deleteFile(fileName: String): Boolean {
        val file = File(fileName) // 如果文件路径所对应的文件存在，并且是一个文件，则直接删除
    ...
    fun deleteDirectory(dir: String, isDeleteSelf: Boolean): Boolean {
        val dirFile = File(dir) // 如果dir对应的文件不存在，或者不是一个目录，则退出
